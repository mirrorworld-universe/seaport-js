...
import { BigNumberish, Contract, ethers, providers, Wallet } from "ethers";
...
export class SeaportCustom {
  ...
  public async createOrder(
    ...
    const offerer = accountAddress as string;
  ...
  public async signCreateOrderMessage(
    singer: Wallet,
    msg: string
  ): Promise<string> {
    const data = JSON.parse(msg);

    const sig = await singer._signTypedData(
      data.domain,
      EIP_712_ORDER_TYPE,
      data.message
    );

    return ethers.utils.splitSignature(sig).compact;
  }
